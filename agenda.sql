...
CREATE INDEX `fk_foto_nome_arquivo_idx` ON `agenda`.`foto` (`nome_arquivo` ASC) ;
...
CREATE  TABLE IF NOT EXISTS `agenda`.`contato` (
  ...
  CONSTRAINT `fk_contato_foto`
    FOREIGN KEY (`foto_id` )
    REFERENCES `agenda`.`foto` (`id` )
    ON DELETE SET NULL
    ON UPDATE CASCADE)
ENGINE = InnoDB;

CREATE INDEX `fk_contato_foto_idx` ON `agenda`.`contato` (`foto_id` ASC) ;
